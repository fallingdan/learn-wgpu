...
use wgpu::{RenderPassColorAttachment, util::DeviceExt};
use winit::{
...
impl State {
    ...
    fn handle_mouse_moved(&mut self, position: PhysicalPosition<f64>) {
        ...
        let r_value = position.x / width;
        let g_value = position.y / height;
        let b_value = (position.x + position.y) / (width + height);

        self.clear_color.r = r_value;
        self.clear_color.g = g_value;
        self.clear_color.b = b_value;
    }
}
